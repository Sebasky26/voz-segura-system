...
const registerSchema = zod_1.z.object({
    ...
    rol: zod_1.z.enum(['DENUNCIANTE', 'SUPERVISOR', 'ADMIN']).optional(),
});
...
router.post('/login', async (req, res) => {
    try {
        ...
        if (!user) {
            await (0, logging_1.logToAuditService)('LOGIN_FALLIDO', {
                email,
        ...
        const locked = await (0, auth_2.isUserLocked)(user.id);
        if (locked) {
        ...
        if (!passwordValid) {
            await (0, auth_2.recordFailedLogin)(user.id);
            await (0, logging_1.logToAuditService)('LOGIN_FALLIDO', {
                usuarioId: user.id,
                email,
        ...
        // Login exitoso
        await (0, auth_2.resetFailedLogins)(user.id);
        await (0, logging_1.logToAuditService)('LOGIN_EXITOSO', {
            usuarioId: user.id,
            email,
            ip,
            userAgent
...
router.post('/register', async (req, res) => {
    try {
        ...
        const passwordCheck = (0, auth_2.validatePasswordStrength)(password);
        ...
        // Log de auditoría
        await (0, logging_1.logToAuditService)('REGISTRO_EXITOSO', {
...
router.post('/reset-password', async (req, res) => {
    try {
        ...
        if (user) {
            // Log de auditoría
            await (0, logging_1.logToAuditService)('RESET_PASSWORD_SOLICITADO', {
                usuarioId: user.id,
                email,
            });
